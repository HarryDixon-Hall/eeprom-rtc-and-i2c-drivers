...
void __attribute__((picinterrupt(("")))) I2C_ISR(void);
# 2 "i2c.c" 2




static volatile i2c_request_t queue[4];
...
void I2C1_Init_MasterMode(unsigned long clock_freq) {
    ...
    SSP1STAT = 0b10000000;
    SSP1CON2 = 0x00;
    SSP1ADD = (16000000 / (4 * clock_freq)) - 1;
    TRISCbits.TRISC3 = 1;
    TRISCbits.TRISC4 = 1;
    ANSELCbits.ANSC3 = 0;
    ANSELCbits.ANSC4 = 0;
...
unsigned char I2C1_Submit(const i2c_request_t *request) {
    unsigned char next = (qhead + 1) % 4;
    if (next == qtail) return 1;
    queue[qhead] = *request;
    qhead = next;
    if (!qbusy) {
        qbusy = 1;
...
void __attribute__((picinterrupt(("")))) I2C_ISR(void) {
    ...
    if (cur->rw == 0) {
        if (idx == 0) SSP1CON2bits.SEN = 1;
            ...
            SSP1CON2bits.PEN = 1;
            if (cur->callback) cur->callback(0, 0, 0);
            qtail = (qtail + 1) % 4;
        ...
        if (idx == 0) SSP1CON2bits.SEN = 1;
        ...
        else if (idx == 3) SSP1CON2bits.RSEN = 1;
        else if (idx == 4) SSP1BUF = (cur->slave_addr << 1) | 1;
            ...
            if (cur->callback) cur->callback(cur->buf, cur->len, 0);
            qtail = (qtail + 1) % 4;
            cur = (qtail != qhead) ? &queue[qtail] : 0;
            idx = 0;
            qbusy = (cur != 0);
